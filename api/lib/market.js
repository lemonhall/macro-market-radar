...
export async function getMarketSnapshot({ force = false, now = Date.now() } = {}) {
  ...
  const [yahooSettled, officialSettled, fredSettled] = await Promise.all([
    ...
    Promise.allSettled([fetchTreasuryMetrics(), fetchLiquidityMetrics()]),
    mapWithConcurrency(FRED_METRICS, 2, fetchFredMetric),
  ])
  ...
  const metrics = [...yahoo, ...treasury, ...liquidity, ...fred].map((metric) => withFreshness(metric, now))
  ...
  const snapshot = {
    ...
    sources: [
      ...
      { name: '美国财政部', cadence: '工作日日频', role: '20 年收益率与官方曲线' },
      { name: '纽约联储', cadence: '工作日日频', role: 'SOFR 与隔夜逆回购' },
    ],
    cache: 'fresh',
  }
